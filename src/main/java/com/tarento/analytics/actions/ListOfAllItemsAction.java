...
public class ListOfAllItemsAction implements ActionHelper {
	...
	public List<Data> compute(JsonNode aggrNode, AggregateRequestDto request, Map<String, List<Data>> dataMap) {
		...
		List<Data> outputList = aggrMap.entrySet().stream().sorted(Comparator.comparing(e -> e.getKey()))
				.map(e -> new Data(e.getKey(), e.getValue(), OUTPUT_SYMBOL)).collect(Collectors.toList());
		return outputList;
	}
